...
Group* SceneParser::parseGroup() {
	...
	while (num_objects > count) {
		...
		if (!strcmp(token, "MaterialIndex")) {
			...
			auto object = unique_ptr<Object3D>(parseObject(token));
			assert(object);
			answer->insert(move(object));
			count++;
		}
	}
	getToken( token ); assert (!strcmp(token, "}"));
...
Group* SceneParser::parseTriangleMesh() {
	...
	while (1) {
		...
		if (c == EOF) { break;
			...
			assert (current_material != nullptr);
			auto t = make_unique<Triangle>(verts[f0-1], verts[f1-1], verts[f2-1], current_material);
			answer->insert(move(t));
			new_fcount++; 
		} // otherwise, must be whitespace
	}
	delete [] verts;
